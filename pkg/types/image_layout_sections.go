...
// ImageSignature represents the image signature structure
// Based on image_layout_image_signature from mstflint
type ImageSignature struct {
	SignatureType uint32    // offset 0x0
	Signature     [256]uint8 // offset 0x4
}

// ImageSignature2 represents the extended image signature structure
// Based on image_layout_image_signature_2 from mstflint
type ImageSignature2 struct {
	SignatureType uint32    // offset 0x0
	Signature     [512]uint8 // offset 0x4
}

// PublicKey represents a public key structure
// Based on image_layout_file_public_keys from mstflint
type PublicKey struct {
	Reserved uint32    // offset 0x0
	UUID     [16]uint8 // offset 0x4
	Key      [256]uint8 // offset 0x14
}

// PublicKeys represents an array of public keys
// Based on image_layout_public_keys from mstflint
type PublicKeys struct {
	Keys [8]PublicKey // offset 0x0
}

// PublicKey2 represents an extended public key structure
// Based on image_layout_file_public_keys_2 from mstflint
type PublicKey2 struct {
	Reserved uint32    // offset 0x0
	UUID     [16]uint8 // offset 0x4
	Key      [512]uint8 // offset 0x14
}

// PublicKeys2 represents an array of extended public keys
// Based on image_layout_public_keys_2 from mstflint
type PublicKeys2 struct {
	Keys [8]PublicKey2 // offset 0x0
}

// ToolsAreaExtended represents the tools area structure
// Based on image_layout_tools_area from mstflint
type ToolsAreaExtended struct {
	TLVRC   uint32    // offset 0x0
	CRCFlag uint32    // offset 0x4
	TotalLength uint32 // offset 0x8
	TypeLength uint32  // offset 0xc
	TypeData   [16]uint8 // offset 0x10
	Reserved   [32]uint8 // offset 0x20
}