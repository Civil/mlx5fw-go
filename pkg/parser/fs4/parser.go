...
func (p *Parser) Parse() error {
	...
	// Parse ITOC - first try standard location, then alternate location
	if err := p.parseITOC(); err != nil {
		p.logger.Debug("ITOC parsing failed at standard location, trying alternate location", zap.Error(err))
...
func (p *Parser) parseHWPointers() error {
	...
	p.logger.Debug("Parsed HW pointer values",
		...
		zap.String("hashes_table_ptr_hex", fmt.Sprintf("0x%x", p.hwPointers.HashesTablePtr.Ptr)))
...
func (p *Parser) parseITOC() error {
	...
	// Use the generic TOC reader for sections
...
func (p *Parser) parseDTOC() error {
	...
	sections, err := p.tocReader.ReadTOCSectionsNew(firmwareData, p.dtocAddr, true)
	if err != nil {
		return err
	}
...
// GetSections returns all parsed sections using the new interface
func (p *Parser) GetSections() map[uint16][]interfaces.CompleteSectionInterface {
	return p.sections
}
...
// SetFormat sets the firmware format
func (p *Parser) SetFormat(format types.FirmwareFormat) {
	p.format = format
}
...
func (p *Parser) VerifySection(section *interfaces.Section) (string, error) {
	...
	// Check if section has CRC verification enabled (no_crc flag)
	if section.Entry != nil && section.Entry.GetNoCRC() {
		return "CRC IGNORED", nil
	}

	// Debug: Log CRC info for certain sections
	if section.Type == types.SectionTypeImageSignature256 || section.Type == types.SectionTypeImageSignature512 ||
		section.Type == types.SectionTypeFwNvLog || section.Type == types.SectionTypeHMACDigest ||
		section.Type == types.SectionTypeNvData0 || section.Type == types.SectionTypeNvData2 {
		if section.Entry != nil {
			p.logger.Info("Section CRC debug info",
				zap.String("type", types.GetSectionTypeName(section.Type)),
				zap.Uint8("crc_field", section.Entry.GetCRC()),
				zap.Bool("no_crc", section.Entry.GetNoCRC()),
				zap.String("crc_binary", fmt.Sprintf("%03b", section.Entry.GetCRC())))
		}
	}
...
func (p *Parser) parseBoot2() error {
	...
	// Boot2 size is at offset 4
	size := binary.BigEndian.Uint32(headerData[4:8])
	// Prevent integer overflow in (size + 4) * 4 calculation
	const maxBoot2SizeDwords = 0x3FFFD // (0x3FFFD + 4) * 4 = 0xFFFFC
	if size == 0 || size > maxBoot2SizeDwords {
		return merry.Errorf("invalid BOOT2 size: %d dwords", size)
	}
	...
	// Read the full BOOT2 data including CRC
	readSize := boot2SizeBytes
	if !p.isEncrypted {
...
func (p *Parser) parseHashesTable() error {
	...
	if p.hwPointers.HashesTablePtr.Ptr == 0 || p.hwPointers.HashesTablePtr.Ptr == 0xffffffff {
		p.logger.Debug("No valid HASHES_TABLE pointer",
			zap.Uint32("ptr", p.hwPointers.HashesTablePtr.Ptr))
	...
	// Check if HASHES_TABLE section already exists from ITOC parsing
	existingSections := p.sections[types.SectionTypeHashesTable]
	for _, existingSection := range existingSections {
		if existingSection.Offset() == uint64(hashesTableAddr) {
			p.logger.Debug("HASHES_TABLE section already exists from ITOC parsing",
				zap.Uint64("offset", existingSection.Offset()),
				zap.Uint32("size", existingSection.Size()))
			return nil
		}
	}

	// HASHES_TABLE section not found in ITOC, need to create it from HW pointer
	...
	p.logger.Info("Found HASHES_TABLE section from HW pointer",
		zap.Uint64("offset", section.Offset()),
		zap.Uint32("size", section.Size()))
...
// tryAlternateITOCLocation tries to parse ITOC at alternate location (offset + 0x1000)
func (p *Parser) tryAlternateITOCLocation() error {
	const FS4_DEFAULT_SECTOR_SIZE = 0x1000
	alternateItocAddr := p.itocAddr + FS4_DEFAULT_SECTOR_SIZE

	p.logger.Debug("Trying alternate ITOC location",
		zap.Uint32("original_addr", p.itocAddr),
		zap.Uint32("alternate_addr", alternateItocAddr))

	// Try reading ITOC header from alternate location
	...
	// Check if we found valid ITOC
	if tempHeader.Signature0 != types.ITOCSignature {
		return merry.New("no valid ITOC at alternate location")
	}
	...
	// Update ITOC address to the alternate location
	p.itocAddr = alternateItocAddr

	// Now parse ITOC normally from the new location
	return p.parseITOC()
}
	...
	if tempHeader.Signature0 == types.ITOCSignature {
		...
		for i := 0; i < maxEntries; i++ {
			...
			if err := entry.Unmarshal(entryData); err != nil {
				p.logger.Warn("Failed to unmarshal ITOC entry", zap.Error(err))
				continue
			}
			...
			if entry.GetFlashAddr() > 0x10000000 { // 256MB limit
				continue
			}

			// Debug HASHES_TABLE sections
			if entry.GetType() == 0xFA {
				p.logger.Info("HASHES_TABLE section found",
			...
			section, err := p.sectionFactory.CreateSection(
				entry.GetType(),
				uint64(entry.GetFlashAddr()),
				entry.GetSize(),
				p.tocReader.GetCRCTypeLegacy(entry),
			...
			if err != nil {
				p.logger.Warn("Failed to create section", zap.Error(err))
				continue
			}
		...
		if p.hwPointers.ImageInfoSectionPtr.Ptr != 0 && p.hwPointers.ImageInfoSectionPtr.Ptr != 0xffffffff {
			...
			section, err := p.sectionFactory.CreateSection(
				...
				false, // isDeviceData
				nil,   // entry
				true,  // isFromHWPointer
			)
			if err != nil {
				p.logger.Warn("Failed to create IMAGE_INFO section", zap.Error(err))
			} else {
				p.addSection(section)
			}
		}
...
func (p *Parser) parseEncryptedFirmwareMinimal() error {
	...
	if p.hwPointers.ImageInfoSectionPtr.Ptr != 0 && p.hwPointers.ImageInfoSectionPtr.Ptr != 0xffffffff {
		...
		section, err := p.sectionFactory.CreateSection(
			...
			false, // isDeviceData
			nil,   // entry
			true,  // isFromHWPointer - mark as from HW pointer in encrypted firmware
		)
		if err != nil {
			p.logger.Warn("Failed to create IMAGE_INFO section", zap.Error(err))
		} else {
	...
	if p.boot2Addr != 0 && p.boot2Addr != 0xffffffff {
		...
		if err == nil {
			...
			if size > 0 && size < 0x100000 { // Sanity check
				...
				if err != nil {
					p.logger.Warn("Failed to create BOOT2 section", zap.Error(err))
				} else {
					p.addSection(section)
				}
	...
	if toolsAreaAddr != 0 {
		...
		if err == nil {
			...
			if err != nil {
				p.logger.Warn("Failed to create TOOLS_AREA section", zap.Error(err))
			} else {
				p.addSection(section)
			}
